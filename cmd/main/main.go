...
func handleCallback(bot *tgbotapi.BotAPI, callback *tgbotapi.CallbackQuery) {
    ...
    default:
        ...
        if docker.IsContainerAction(callbackData) {
            ...
            handleContainerAction(bot, callback, action, containerID)
            return // Прерываем выполнение, чтобы не обновлять сообщение
    ...
    if editMsg.Text != callback.Message.Text || !compareInlineKeyboard(editMsg.ReplyMarkup, callback.Message.ReplyMarkup) {
        ...
        log.Printf("Message is not modified, skipping update")
    }
}
...
func handleContainerAction(bot *tgbotapi.BotAPI, callback *tgbotapi.CallbackQuery, action string, containerID string) {
    var result string
    ...
    // Отправляем уведомление через поп-ап аннотацию
    callbackAnswer := tgbotapi.NewCallback(callback.ID, result)
